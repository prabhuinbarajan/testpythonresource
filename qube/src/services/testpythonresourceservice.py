...
class TestPythonResourceService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = TestPythonResource.query.get(entity_id)
        if data is None:
            raise TestPythonResourceServiceError(
                'testpythonresource {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = TestPythonResource.query.filter(
            TestPythonResource.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = TestPythonResource.query.get(entity_id)  # TestPythonResource is a mongo class
        if record is None:
            raise TestPythonResourceServiceError(
                'testpythonresource {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
